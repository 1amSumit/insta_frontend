...
function MobileTopSearchBar() {
  ...
  return (
    <>
      ...
      {isLoading ? (
        <div
          className={`users border-t-[1px] absolute top-10 right-3 bg-gray-800 z-40 px-[3rem] py-[1rem] flex flex-col h-auto gap-[1rem] border-gray-600 rounded-lg overflow-y-auto no-scrollbar ${
            data === null && "hidden"
          }`}
        >
      ...
      {heartClicked === true && (
        <motion.div className="h-[100vh] absolute z-30 top-0 bottom-0 right-0 left-0">
          <AnimatePresence>
            <Notifications
